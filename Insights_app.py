...
ADMIN_USERNAME = "Mzakyi"  # Change this to your preferred admin username
ADMIN_PASSWORD = "Neymar123!@#"  # Change this to your preferred admin password
...
def admin_dashboard():
    ...
    pending_users = sum(1 for u, data in users.items() if not data.get("approved", False) and not data.get("is_admin", False))
    approved_users = sum(1 for u, data in users.items() if data.get("approved", False) and not data.get("is_admin", False))
...
def authenticate():
    ...
    if not st.session_state["logged_in"]:
        ...
        st.title("🔐 Welcome to Sankatos App")
...
# ============= MAIN APP =============
if authenticate():
    # Add logout button and admin panel in sidebar
    with st.sidebar:
    ...
    # Show admin dashboard if admin clicked the button
    if st.session_state.get("show_admin", False) and st.session_state.get("is_admin", False):
        ...
        # [REST OF YOUR ORIGINAL APP CODE GOES HERE - starting from file uploader]
        # You'll need to paste all your original code from the "uploaded_file = st.file_uploader..." line onwards
